          ...
          >
            <div class="flex items-center justify-between mb-3">
              ...
              {:else if pendingCount > 0}
                <span
                  class="text-sm text-blue-700 dark:text-blue-200 bg-blue-100 dark:bg-blue-800 px-2 py-1 rounded-full"
                >
            ...
            {:else}
              <div class="space-y-2">
                {#each incompleteTasks as task (task.id)}
                  ...
                  <div
                    class="flex items-center gap-2 text-sm bg-white dark:bg-gray-800 rounded-md p-2 border border-blue-100 dark:border-blue-800"
                  >
                    <div
                      class="w-3 h-3 border border-blue-400 dark:border-blue-500 rounded"
                    ></div>
                    <span
                      class="text-blue-800 dark:text-blue-200 truncate flex-1"
                      >{task.description}</span
                    >
                    <span
                      class="text-xs px-1.5 py-0.5 rounded-full border flex items-center gap-1 font-medium
          ...
          >
            <div class="flex items-center justify-between mb-3">
              ...
              {:else if expenseCount > 0}
                <span
                  class="text-sm text-red-700 dark:text-red-200 bg-red-100 dark:bg-red-800 px-2 py-1 rounded-full"
                >
            ...
            {:else}
              <div class="space-y-2">
                {#each recentExpenses as expense (expense.id)}
                  <div
                    class="flex items-center justify-between text-sm bg-white dark:bg-gray-800 rounded-md p-2 border border-red-100 dark:border-red-800"
                  >
                    <div class="flex items-center gap-2 flex-1 min-w-0">
                      <div
                        class="w-2 h-2 rounded-full bg-red-500 dark:bg-red-400"
                      ></div>
                      <span class="text-red-800 dark:text-red-200 truncate"

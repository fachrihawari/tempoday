...
<div class="h-screen flex flex-col transition-all duration-500 overflow-hidden">
  ...
  <div
    ...
    style={currentSlideData.customBgStyle}
  ></div>
  <div
    class="absolute inset-0 opacity-0 dark:opacity-90 transition-opacity duration-500"
    style={currentSlideData.darkBgStyle}
  ></div>
    ...
    <div class="flex items-center justify-between px-6 py-6">
      <div class="flex items-center gap-3">
        <div
          ...
          style={isDarkMode
            ? currentSlideData.darkStyle
            : currentSlideData.customStyle}
        >
    ...
    >
      ...
      >
        {#each slides as slide, index (index)}
          <div
            class="w-full h-full flex-shrink-0 flex flex-col items-center justify-center text-center px-8"
          >
            <!-- Large Icon with Beautiful Gradient -->
            <div
              class="w-32 h-32 rounded-3xl flex items-center justify-center shadow-2xl mb-8 transform hover:scale-105 transition-transform duration-300"
              style={isDarkMode
                ? currentSlideData.darkStyle
                : slide.customStyle}
            >
            ...
            <div class="max-w-sm space-y-4">
              <h2
                class="text-3xl font-bold text-gray-900 dark:text-gray-100 leading-tight"
              >
                {slide.title}
              </h2>
        ...
        {#each slides as slide, index}
          ...
          >
            ...
            {#if index !== currentSlide}
              <div
                class="absolute inset-1 bg-gray-400 dark:bg-gray-500 rounded-full"
              ></div>
            {/if}
          </button>
        {/each}
      ...
      <div class="flex gap-4">
        {#if currentSlide > 0}
          <Button
            variant="outline"
            onclick={prevSlide}
            class="flex-1 h-14 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700"
          >
            {#snippet children()}
